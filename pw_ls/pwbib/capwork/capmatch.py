...
"""capmatch.py
   Dec 21, 2015
Usage: python capmatch.py ../pwbib1.txt ../../pw_dhaval/abbrvwork/abbrvoutput/sortedcrefs.txt  capmatch.txt > capmatch_log.txt
"""
...
def adjust_bibrecs_cap(bibrecs):
 recs=[] # returned
 removed=[] 
 for rec in bibrecs:
...
if __name__ == "__main__":
 ...
 for key in crefdict.keys():
  cref = crefdict[key]
  capkey=cref.capabbrv
  if capkey == None:
   continue
  if (key not in bibdict) and (capkey in bibdict):
   if not key.endswith('.'):
    key = key + '.'
   if not capkey.endswith('.'):
    capkey = capkey + '.'

   fout.write(u"¯%s@¯%s@t@capitalization\n" %(key,capkey))
   nout=nout+1
 fout.close()
