...
def changeline(line,abbrs1):
 def f(m):
  ...
  if ls[n] == ' ':
   return ans
  # now ls has additional characters after lsname, but the next character
  # is not a space.
  # if there is only one additional character, which is a period, then no change
  if ls == (lsname + '.'):
   return ans
  # if the next character is a period, replace it with a space
  if ls[n] == '.':
   lsnew = lsname + ' ' + ls[n+1:]
  else:
   # otherwise, insert a space after lsname
   lsnew = lsname + ' ' + ls[n:]
  ans = '<ls>%s</ls>'%lsnew
  return ans
 #
...
if __name__=="__main__":
 ...
 for entry in entries:
  for iline,line in enumerate(entry.datalines):
   newline = changeline(line,abbrs1)
   if newline != line:
