...
def change_regex(line,abbrs1):
 def f(m):
  ...
  lsnum = ls[n+1:]
  # misc changes
  # 1) lsnum1 = lsnum.replace(',Z.', ', Z.')
  # 2) lsnum1 = re.sub(r'([^ ])(fgg?[.])', r'\1 \2',lsnum)
  lsnum1 = lsnum
  # 3) if re.search(r'^2,a[.] [0-9]+,[0-9]+[.]$',lsnum):
  #  lsnum1 = lsnum.replace('a. ','a,')
  # 5) lsnum1 = lsnum.replace(',Śl.', ', Śl.')
  lsnum1 = lsnum.replace(',Śl.', ', Śl.')
  
  if lsnum1 == lsnum:
   return ans
  lsnew = ls[0:n+1]+lsnum1
  ans = '<ls>%s</ls>'%lsnew
  return ans
...
if __name__=="__main__":
 ...
 for entry in entries:
  for iline,line in enumerate(entry.datalines):
   newline = change_regex(line,abbrs1)
   if newline != line:
