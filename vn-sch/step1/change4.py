...
def init_changes(lines):
 ...
 for iline,line in enumerate(lines):
  if line.startswith('<L>'):
   ...
   L = m.group(1)
   # search in subsequent lines for the 'type' parameter
   thetype = None
   j = 0
   while True:
    j = j + 1
    linej = lines[iline+j]
    if linej == '<LEND>':
     break  # a failure. Should not happen
    m = re.search(r'{part=[^}]*?,type=(.*?),.*?}',linej)
    if m == None:
     continue
    thetype = m.group(1)
    break
   if thetype == None:
    print('ERROR Cannot find type',metaline)
    exit(1)
   continue
