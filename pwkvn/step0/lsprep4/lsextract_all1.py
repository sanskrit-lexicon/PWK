#-*- coding:utf-8 -*-
"""lsextract_all1.py -- summary stats
"""
...
def adjust_pwkvn(line):
 line = re.sub(r'(-?<lb/>)|(-?\[Page.*?\])','',line)
 return line
...
def count_tips(lines,tipd,numbertip,unknowntip):
 ...
 for iline,line in enumerate(lines):
  ...
  if line.startswith('[Page'):
   page = line
   continue
  # remove some interfering code (line breaks, etc.) from code
  line1 = adjust_pwkvn(line)
  for m in re.finditer(r'<ls([^>]*)>([^<]*)</ls>',line1):
   attrib = m.group(1)
   elt = m.group(2)
   if len(elt) == 0:
   ...
   if re.search(r'^[0-9]',elt): # number
    ...
    if tip == None:
     tip = unknowntip
     print('line# %s unknown tip %s' %(iline+1,elt))
   # found a match
...
def write_tips(fileout,tips0,numbertip,unknowntip):
 ...
 #date1 = 'As of %s' % date
 alltip = Tooltip("9.0\tALL\tall\tAs of %s"%date)
 ...
 def tipformat(tip):
  ...
  #text = text[0:40]
  #return '%05d\t%s\t%s\t%s\t%s' %(tip.total,tip.abbrev,tip.code,tip.abbrevlo,text)
  return '%05d\t%s' %(tip.total,tip.line)
 ...
 for tip in tips:
  ...
  tot = tot + tip.total
 #
 #outrecs[0] = '%05d\t%s\t%s\t%s\t%s' %(tot,'ALL','_','all',date1)
 alltip.total = tot
 outrecs[0] = tipformat(alltip)
 with codecs.open(fileout,"w","utf-8") as f:
  for out in outrecs:
   f.write(out+'\n')
