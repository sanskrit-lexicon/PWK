...
class Pwls:
 def __init__(self,line):
  ...
  parts = line.split('\t')
  self.count = parts[0]
  self.tiprec = '\t'.join(parts[1:])
  self.code,self.lsiast,self.abbrevlo,self.tooltip = self.tiprec.split('\t')
  #self.count,self.lsiast,self.code,self.abbrevlo,self.tooltip = line.split('\t')
 ...
 with codecs.open(fileout,"w","utf-8") as f:
  for vnrec in vnrecs:
   ...
   if pwrec == None:
    count = '00000'
    abbrev = vnrec.lsiast
    words = re.split(r' +',abbrev)
    wordscap = [w.capitalize() for w in words]
    abbrevlo = ' '.join(wordscap)
    title = 'NONE'
    nprob = nprob + 1
    code = 'v%04d' % nprob
   else:
    count = pwrec.count
    abbrev = pwrec.lsiast
    abbrevlo = pwrec.abbrevlo
    title = pwrec.tooltip
    code = pwrec.code
    nok = nok + 1
   out = '%s\t%s\t%s\t%s\t%s\t%s\t%s' %(vnrec.count,count,vnrec.ls,code,abbrev,abbrevlo,title)
   f.write(out+'\n')
...
def write_unused(fileout,pwrecs):
 ...
 with codecs.open(fileout,"w","utf-8") as f:
  for pwrec in pwrecs1:
   if pwrec.used != None:
    continue
   out = '%s\t%s\t%s\t%s\t%s' % (pwrec.count,pwrec.ls,pwrec.code,pwrec.abbrevlo,pwrec.tooltip)
   nprob = nprob + 1
   f.write(out+'\n')
