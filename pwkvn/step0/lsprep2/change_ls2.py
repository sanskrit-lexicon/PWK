...
def unused_part_to_lsname(part):
...
def lsparts_to_lsheads(lsparts,dbg=False):
 ...
 for fraglen in reversed(fraglens):  # go from longest to shortest
  ...
  if lsname in otherlsnames:
   unused_parts = lsparts[fraglen:]
   return lsheads,lsname,unused_parts
 # Try 'normal'
...
def part_to_lsname1(part0,dbg=False):
 ...
 if part1 != part:
  ...
  exit(1)
 lsheads,lsname,lsrest = lsparts_to_lsheads(lsparts1,dbg=dbg)
 lsnewparts = lsparts1[0:len(lsheads)] + ['</ls>'] + lsrest
 newpart = ''.join(lsnewparts)
 if dbg:
  for ix,x in enumerate(lsparts1):
   if x != None:
...
def count_ls1(lines):
 ...
 for iline,line0 in enumerate(lines):
  ...
  for ipart,part in enumerate(parts):
   if part == '<ls>':
    ...
    continue
   if prev == None: # first case
    newparts.append(part)
    prev = 'FIRST'
    continue
   # part is the string following some <ls>
   ## begin computation of lsname from part
   lsname,newpart = part_to_lsname1(part)
 ...
 return lsnamed,exceptions,newlines
...
if __name__=="__main__":
 ...
 fileout = sys.argv[2] # ls2.txt
 filenew = sys.argv[3] # new file with </ls> added.
 with codecs.open(filein,"r","utf-8") as f:
  lines = [x.rstrip('\r\n') for x in f]
 print(len(lines),'lines read from',filein)
 #plines = [line for line in lines if line.startswith('<p>')]
 #print(len(plines),'lines start with <p>')
 lscountd,exceptions,newlines = count_ls1(lines)
