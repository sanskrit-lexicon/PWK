#!/usr/bin/env python3
# Build consolidated single-file editions of the PW (Böhtlingk, Sanskrit-Wörterbuch
# in kürzerer Fassung, 1879) front matter from the per-page pwprefNN.* files.
# Data-driven: reads each page's YAML — no hardcoded page list.
# Run from the prefaces/ directory:  python build_combined.py   (or:  DICT=pw python build_combined.py)
import sys, os, re, glob
sys.stdout.reconfigure(encoding='utf-8'); sys.stderr.reconfigure(encoding='utf-8')
HERE = os.path.dirname(os.path.abspath(__file__))
CODE = os.environ.get('DICT', 'pw')
...
for lang,(suf,outname,title,pw,srcw) in LANGS.items():
    ...
    for de in pages:
        ...
        txt=re.sub(r'(?m)^(#{1,5})(\s)', r'#\1\2', txt)  # demote in-body headings
