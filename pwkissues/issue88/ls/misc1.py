...
def adjust(lines):
 ...
 for iline,line in enumerate(lines):
  if iline == 0: 
   ...
   newline = re.sub(r'<ls>VP\.² 1,([A-Z])',   r'<ls>VP.² 1, \1',line)
   newline = newline.replace(').</ls>' ,  ')</ls>.')
   newline = newline.replace('</ls> im <ls>ŚKDR.</ls>',
                             ' im ŚKDR.</ls>')
   newline = re.sub(r'</ls> (\(S\..*?\))',  r' \1</ls>',newline)
  if newline != line:
   nchg = nchg + 1 
