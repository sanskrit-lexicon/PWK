...
class Tip(object):
 def __init__(self,abbrevtuple):
  ...
  self.abbrev = '<is n="%s">%s</is>' %(self.tip,self.abbrev_string)
...
def make_outarr(tips0):
 ...
 outarr = []
 ndiff = 0 # number with different counts
 for rec in recs:
  if rec.usedcd == rec.usedab:
   out = '%s\t%s,%s' %(rec.abbrev,rec.usedcd,rec.usedab)
  else:
   ndiff = ndiff + 1
   comment = 'Different counts'
   out = '%s\t%s,%s\t%s' %(rec.abbrev,rec.usedcd,rec.usedab,comment)
  outarr.append(out)
 print(ndiff,"instances with different counts")
 return outarr
