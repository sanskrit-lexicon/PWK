# coding=utf-8
""" regex_compare_texts_count2.py
 Examine only lines matching a given regex
"""
...
def compare_tags(text1,text2,metaline,regex):
 ...
 try: 
  ...
  print('compare_tags dbg: regex=',regex)
  print('text1=',text1)
  exit(1)
 a2 = re.findall(regex,text2)
 ans = []
...
def compare_filter_lines(lines,regexline):
 newlines = []
 for line in lines:
  m = re.search(regexline,line)
  if m!= None:
   newlines.append(line)
 return newlines
 
def compare(entries1,entries2,maxdiff,regex,regexline):
 dbg = False
 nd = 0
 ntag = 0
 tagtype = None
 ...
 for ientry,e1 in enumerate(entries1):
  e2 = entries2[ientry]
  lines1 = compare_filter_lines(e1.datalines,regexline)
  text1 = ' '.join(lines1)
  lines2 = compare_filter_lines(e2.datalines,regexline)
  text2 = ' '.join(lines2)
...
if __name__=="__main__":
 regex = sys.argv[1]
 regexline = sys.argv[2]
 filein = sys.argv[3] # xxx.txt cdsl
 filein1 = sys.argv[4] # xxx.txt AB
 fileout = sys.argv[5] #
 if len(sys.argv) == 8:
  # optional output
  fileout_xtra = sys.argv[6]
  fileout1_xtra = sys.argv[7]
  xtraflag = True
 else:
  xtraflag = False
 ...
 maxdiff = None
 outrecs = compare(entries_cdsl,entries_ab,maxdiff,regex,regexline)
