...
def update_tips(tips,dtips,src,dabbrev):
 ...
 for abbrev in dabbrev:
  ...
  if True: print('dbg: src=%s, abbrev=%s, count=%s' %(src,abbrev,count))
...
if __name__=="__main__":
 ...
 d = count_lex(lines)
 print(len(d),"distinct <lex>X</lex> from",filein)
 #
 lines1 = read_lines(filein1)
 print(len(lines1),"lines from",filein1)
 d1 = count_lex(lines1)
 print(len(d1),"distinct <lex>X</lex> from",filein1)
 # tips
