...
def init_words_page(filein):
 ...
 return d
def make_outrecs(lines,word_links_d):
 """ two forms:
* Engste 1 -DE IT 0 : ? : JF : 
engsten 1 +DE IT 0 : ? : 
 """
 ...
 for line in lines:
  ...
  for link in links:
   linkname = link.pc
   linkhref = link.href
   # Emacs org mode [[href][name]]
   linkout = '[[%s][%s]]' % (linkhref,linkname)
   out = '; %s %s' %(linkout,link.metaline)
   outarr.append(out)
