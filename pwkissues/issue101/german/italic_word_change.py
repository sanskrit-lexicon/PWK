...
class Input:
 def __init__(self,old,new,inputcount):
  ...
  self.changecount = 0  # filled in by compare function
...
def compare(entries,dinput):
 dbg = False
 changes = []
 for ientry,e in enumerate(entries):
  for iline,line in enumerate(e.datalines):
   newline,inputs_applied = line_changes(line,dinput)
   if newline == line:
    continue # no changes on this line
   # construct change record for this line
   lnum = e.linenum1 + iline + 1
   metaline = e.metaline
   oldline = line
   change = Change(metaline,lnum,oldline,newline,inputs_applied)
...
def write_newinputs(fileout,inputrecs):
 ...
 for rec in inputrecs:
  out = '%s %s %s -> %s' %(rec.inputcount, rec.changecount,rec.old,rec.new)
  outarr.append(out)
...
if __name__=="__main__":
 ...
 fileout1 = sys.argv[4] # inputs with observed
 dinput,inputrecs = init_correction_input(filein1)
 entries_cdsl = digentry.init(filein)
 changes = compare(entries_cdsl,dinput)
 write_changes(fileout,changes,dinput)
 write_newinputs(fileout1,inputrecs)
