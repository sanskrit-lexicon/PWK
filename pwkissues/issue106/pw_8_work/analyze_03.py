...
def get_newgroups(group):
 ...
 if len(parts) != 2:
  print('get_newgroups ERROR:',group[0])
  exit(1)
 #assert len(parts) == 2
 l1 = lnums0[0]
 l2 = lnums0[1]
 ...
 for i,lnum in enumerate(lnums):
  ...
  line3 = group[-1]  # ----
  """ not applicable for non-metalines
...
def make_changes_00(lines,groups):
 d = {}
 for group in groups:
  val = (len(group.ab) - len(group.cdsl))
  if val < -4:
   val = -5
  elif val > 4:
   val = 5
