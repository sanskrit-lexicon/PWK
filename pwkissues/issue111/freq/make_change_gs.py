...
class Rec:
 def __init__(self,line):
  ...
  self.gsinfo = m.group(3)
  self.rest = m.group(4)
  self.used = False
  dbg = line.startswith('<bot>Silurus pungentissi')
  dbg = False
  if dbg :print('line=',line,'\nrest=',self.rest)
  if self.rest.startswith('zoo'):
   self.newelt = '<zoo>%s</zoo>' % self.bot
   self.type = 'bot -> zoo'
   return
  m = re.search(r'bot sp\. +"(.*)"',self.rest)
  if m != None:
   newbot = m.group(1)
   self.newelt = "<bot>%s</bot>" % newbot
   self.type = 'bot spelling change'
   return
  if self.rest.startswith('notbot'):  # not a scientific math name
   self.newelt = '%s' % self.bot
   self.type = self.rest
   # print('Rec material: %s' % line)
   return
  self.newelt = None
  self.type = 'manual'
def init_recs(lines):
...
if __name__=="__main__":
 ...
 changes1 = sorted(changes, key = lambda c: (c.comments[0],c.ilnum))
 write_changes(fileout,changes1)
