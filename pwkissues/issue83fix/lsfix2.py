...
def write_instances(fileout,instances):
 ...
 for instance in instances:
  ...
  out = '%s\t%s\t%s\t%s\t%s\t%s' % (status,nparm,lnum,k1,instance.matchstr,parse)
  outarr.append(out)
  stat1 = (status,nparm)
...
def write_instances_1(fileout,instances):
 outarr = []
 statd = {}
 instances_sorted = sorted(instances,key = lambda x: x.matchstr)
 for instance in instances_sorted:
  ...
  # out = '%s\t%s\t%s\t%s\t%s' % (status,nparm,lnum,instance.matchstr,parse)
  # out = '%s\t%s\t%s\t%s\t%s\t%s' % (status,nparm,lnum,k1,instance.matchstr,parse)
  out = '%s\t%s\t%s\t%s\t%s\t%s' % (status,nparm,lnum,Lk1,instance.matchstr,parse)
  outarr.append(out)
  stat1 = (status,nparm)
  if stat1 not in statd:
   statd[stat1] = 0
...
if __name__=="__main__":
 ...
 outopt = None
 if 'outopt' in targetparms:
  outopt = targetparms['outopt']
 if outopt == None:
  statd = write_instances(fileout,instances_all_select)
 elif outopt == '1':
  statd = write_instances_1(fileout,instances_all_select)
 else:
  print('Using default output option')
  statd = write_instances(fileout,instances_all_select)
 statout = statd_to_statout(statd)
 print(statout,fileout)
